...
#define nn_pop_extension_(protocol, ...) \
        ...
        class NSObject; \
        \
        nn_pop_extension_where_(nn_pop_extension_prefix, protocol, __VA_ARGS__)\
        \
        nn_pop_extension_section_(nn_pop_extension_prefix, protocol, __VA_ARGS__) \
        \
        metamacro_if_eq(0, metamacro_argcount(__VA_ARGS__))(nn_pop_extension_0(nn_pop_extension_prefix, protocol))\
        (metamacro_if_eq(1, metamacro_argcount(__VA_ARGS__))(nn_pop_extension_0(nn_pop_extension_prefix, protocol))\
        (nn_pop_extension_N(nn_pop_extension_prefix, protocol, __VA_ARGS__)))\


#define nn_pop_extension_0(prefix, protocol) \
        \
        @interface nn_pop_extension_name_(nn_pop_extension_prefix, protocol) : NSObject < protocol > \
        \
        @end \
        \
        @implementation nn_pop_extension_name_(nn_pop_extension_prefix, protocol) \
